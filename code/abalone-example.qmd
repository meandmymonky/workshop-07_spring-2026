...
# read in data from excel (HOLY PEAK)
# pull 'Abalone IMTA_growth and pH.xlsx' from 'data' file using 'here' function
...
H~0~: there is no effect of pH on abalone growth (don't put significance in hypothesis, just for common phrasing)  
...
# create new object from 'abalone' data set
...
# base layer: ggplot
ggplot(data = abalone_clean, # starting data frame
       aes(x = mean_ph, # x-axis: pH, y-axis: shell surface area change
           y = change_in_area)) +
...
**The missing value comes from a pair of tanks in which the pH sensor malfunctioned, so there is no pH reading from those tanks.**  
...
# create linear object called abalone_model object
abalone_model <- lm(
...
At pH = 8, the model predicts abalone shell growth of 5.36 (95% CI: 5.08, 5.64) mm^2^
